...
def resnet(n_layers, stride):
    ...
    pretrained_path = {
        50:  osp.join(settings.MODEL_DIR, 'resnet50-25c4b509.pth'),
        101: osp.join(settings.MODEL_DIR, 'resnet50-25c4b509.pth'),
        152: osp.join(settings.MODEL_DIR, 'resnet50-25c4b509.pth'),
    }[n_layers]
...
def test_net():
    model = HamNet(n_classes=21, n_layers=50).cuda()
    model.eval()
    print(list(model.named_children()))
    image = torch.randn(1, 3, 513, 513).cuda()
    label = torch.zeros(1, 513, 513).long().cuda()
