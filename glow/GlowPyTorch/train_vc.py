...
from datasets import get_CIFAR10, get_CXR, get_Fundus, get_corpus
...
def check_dataset(dataset, dataroot, augment, download):
    ...
    if dataset == "corpus":
        corpus = get_corpus()
        input_size, num_classes, train_dataset, test_dataset = corpus
...
def main(
    ...
    device = "cpu" if (not torch.cuda.is_available() or not cuda) else "cuda:3"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="corpus",
        choices=["cifar10", "CXR", "Fundus", "corpus"],
        help="Type of the dataset to be used.",
    )

    parser.add_argument("--dataroot", type=str, default="/data/pycode/SFSAttention/glow/GlowPyTorch/", help="path to dataset")
    ...
    #nohup python train_vc.py > logs/train.log 2>&1 &
